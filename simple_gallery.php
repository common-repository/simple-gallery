...
/*
	...
	Description: Flash Simple gallery. Very easy to use. Load your images(jpg, gif, png) in: wp-content/plugins/simple-gallery/images/
	Version: 1.1
	Author: Webpsilon
	Author URI: http://www.webpsilon.com/wordpress-plugins
*/	
$contador=0;
function simple_gallery_head() {
	
	$site_url = get_option( 'siteurl' );
			echo '<script src="' . $site_url . '/wp-content/plugins/simple-gallery/Scripts/swfobject_modified.js" type="text/javascript"></script>';
			
}
function simple_gallery($content){
	$content = preg_replace_callback("/\[simple_gallery ([^]]*)\/\]/i", "simple_gallery_render", $content);
	return $content;
	
}

function simple_gallery_render($tag_string){
$contador=rand(9, 9999999);
	$site_url = get_option( 'siteurl' );
global $wpdb; 	
$table_name = $wpdb->prefix . "simple_gallery";	
...
if(count($myrows)<1) $myrows = $wpdb->get_results( "SELECT * FROM $table_name;" );
  ...
  <param name="movie" value="'.$site_url.'/wp-content/plugins/simple-gallery/simple-gallery.swf" />
  ...
  <param name="expressinstall" value="'.$site_url.'/wp-content/plugins/simple-gallery/Scripts/expressInstall.swf" />
  <!-- Next object tag is for non-IE browsers. So hide it from IE using IECC. -->
  <!--[if !IE]>-->
  <object type="application/x-shockwave-flash" data="'.$site_url.'/wp-content/plugins/simple-gallery/simple-gallery.swf" width="'.$width.'" height="'.$height.'">
    <!--<![endif]-->
    <param name="quality" value="high" />
    <param name="wmode" value="transparent" />
	  <param name="scale" value="exactfit" />
...
function simple_gallery_instala(){
	global $wpdb; 
	$table_name= $wpdb->prefix . "simple_gallery";
   $sql = " CREATE TABLE $table_name(
	...
	$wpdb->query($sql);
	$sql = "INSERT INTO $table_name (folder, timeback,  onover, links, width, height, title) VALUES ('wp-content/plugins/simple-gallery/images/', '5', '1', '', '100%', '250px', 'Webpsilon WP Plugins');";
	$wpdb->query($sql);
}
function simple_gallery_desinstala(){
	global $wpdb; 
	$table_name = $wpdb->prefix . "simple_gallery";
	$sql = "DROP TABLE $table_name";
	$wpdb->query($sql);
}	
function simple_gallery_panel(){
	global $wpdb; 
	$table_name = $wpdb->prefix . "simple_gallery";	
...
if(empty($re))
{
  $sql = " CREATE TABLE $table_name(
	...
	$wpdb->query($sql);
		...
		$sql = "INSERT INTO $table_name (folder, timeback,  onover, links, width, height, title) VALUES ('wp-content/plugins/simple-gallery/images/', '5', '1', '', '100%', '250px', 'Webpsilon flash easy gallery');";
	$wpdb->query($sql);
	}
...
function simple_gallery_add_menu(){	
	if (function_exists('add_options_page')) {
		//add_menu_page
		add_options_page('simple_gallery', 'Simple gallery', 8, basename(__FILE__), 'simple_gallery_panel');
	}
}
if (function_exists('add_action')) {
	add_action('admin_menu', 'simple_gallery_add_menu'); 
}
add_action('wp_head', 'simple_gallery_head');
add_filter('the_content', 'simple_gallery');
add_action('activate_simple_gallery/simple_gallery.php','simple_gallery_instala');
add_action('deactivate_simple_gallery/simple_gallery.php', 'simple_gallery_desinstala');
?>